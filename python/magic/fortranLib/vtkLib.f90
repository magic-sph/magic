module vtklib
   ...
   use iso_c_binding, only: sp => c_float, dp => c_double, lip => c_long
...
contains
   ...
   subroutine pvts_scal(filename,radius,scalars,scalcodes,nfiles,minc,np,nt,nr,nscals)
      ...
      real(sp),          intent(in) :: radius(nr)
      real(sp),          intent(in) :: scalars(nscals,np,nt,nr)
      integer,           intent(in) :: scalcodes(nscals)
      integer,           intent(in) :: nfiles

      ! local variables
      character(len=20) :: scalnames(nscals), vecnames(0)
      integer :: veccodes(0)
      real(sp) :: xyz(3,np*(nt/nfiles+1)*nr)
      real(sp) :: scals(nscals,np*(nt/nfiles+1)*nr)
      real(sp) :: vecs(0,0,0)
      real(sp) :: theta(nt)
      real(sp) :: phi(np)
      real(sp) :: pi
      ...
      do m=1,nfiles
         ...
         buffer = '    <Piece Extent="'//str1//'" Source="'//trim(fName)//'">'//lf
         write(ivtk) trim(buffer)
         buffer = '    </Piece>'//lf
         write(ivtk) trim(buffer)
      end do
...
!------------------------------------------------------------------------------
   subroutine pvts(filename,radius,vecr,vect,vecp,scalars,scalcodes,veccodes,&
      ...
      real(sp),          intent(in) :: scalars(nscals,np,nt,nr)
      real(sp),          intent(in) :: vecr(nvecs,np,nt,nr)
      real(sp),          intent(in) :: vect(nvecs,np,nt,nr)
      real(sp),          intent(in) :: vecp(nvecs,np,nt,nr)
      integer,           intent(in) :: nfiles
      ...
      real(sp) :: xyz(3,np*(nt/nfiles+1)*nr)
      real(sp) :: scals(nscals,np*(nt/nfiles+1)*nr)
      real(sp) :: vecs(nvecs,3,np*(nt/nfiles+1)*nr)
      real(sp) :: theta(nt)
      real(sp) :: phi(np)
      real(sp) :: pi,vs
      integer :: nnos
...
!------------------------------------------------------------------------------
   subroutine partXMLVts(filename,xyz,scals,scalnames,nscals,vecs,vecnames,nvecs,&
      ...
      integer :: nnos,i,j,k,intSize,nscals,nvecs
      integer :: nr,thinit,thend,nphi

      character(len=*), intent(in) :: filename
      real(sp),         intent(in) :: xyz(3,nnos)
      real(sp),         intent(in) :: scals(nscals,nnos)
      real(sp),         intent(in) :: vecs(nvecs,3,nnos)
      ...
      nbytes_scal = nnos*int(sizeof(floatSize))
      nbytes_vec  = 3*nnos*int(sizeof(floatSize))
      ...
      do i=1,nscals
         ...
         write(ivtk) trim(buffer)
         ioff=ioff+int(sizeof(intSize))+nbytes_scal
      end do
      do i=1,nvecs
         write(offset(1:12),'(i12)') ioff
         buffer = '         <DataArray type="Float32" Name="'&
         ...
         write(ivtk) trim(buffer)
         ioff=ioff+int(sizeof(intSize))+nbytes_vec
      end do
      buffer = '      </PointData>'//lf
      write(ivtk) trim(buffer)
...
!------------------------------------------------------------------------------
   subroutine vts(filename,radius,vecr,vect,vecp,scalars,scalcodes,veccodes,&
      ...
      !-- Local variables
      character(len=20) :: scalnames(nscals),vecnames(nvecs)
      real(sp) :: xyz(3,np*nt*nr)
      real(sp) :: scals(nscals,np*nt*nr)
      real(sp) :: vecs(nvecs,3,np*nt*nr)
      real(sp) :: theta(nt)
      real(sp) :: phi(np)
      real(sp) :: pi,Bs
      integer(lip) :: nnos
...
!------------------------------------------------------------------------------
   subroutine WriteXMLFormat(filename,xyz,scals,scalnames,vecs, &
      ...
      integer(lip) :: ioff
      integer(lip) :: nbytes_scal,nbytes_vec
      integer(lip) :: nnos,i,j,k,intSize
...
!------------------------------------------------------------------------------
   subroutine vti(filename,vecx,vecy,vecz,scalars,scalcodes,veccodes,&
              &   gridMax,spacng,nvecs,nscals,nz,ny,nx)

      !-- Input variables
      integer :: nx,ny,nz,nvecs,nscals
      character(len=64), intent(in) :: filename
      real(sp),          intent(in) :: gridMax,spacng
      real(sp),          intent(in) :: vecx(nvecs,nz,ny,nx)
      real(sp),          intent(in) :: vecy(nvecs,nz,ny,nx)
      real(sp),          intent(in) :: vecz(nvecs,nz,ny,nx)
      real(sp),          intent(in) :: scalars(nscals,nz,ny,nx)
      integer,           intent(in) :: scalcodes(nscals)
      integer,           intent(in) :: veccodes(nvecs)

      !-- Local variables
      character(len=20) :: scalnames(nscals),vecnames(nvecs)
      real(sp) :: scals(nscals,nz*ny*nx)
      real(sp) :: vecs(nvecs,3,nz*ny*nx)
      integer(lip) :: nnos
      integer :: i,j,k,l,iscal,ivec

      nnos = int(nx*ny*nz,lip)
...
!------------------------------------------------------------------------------
   subroutine vti_scal(filename,scalars,scalcodes,gridMax,spacng,nscals,nz,ny,nx)
...
!------------------------------------------------------------------------------
   subroutine WriteXmlVTI(filename,scals,scalnames,vecs,vecnames, &
      ...
      !-- Local variables
      integer(lip) :: nbytes_scal, nbytes_vec, nnos, ioff
      integer(lip) :: i, j, k
      integer :: nscals,nvecs
      character(len=200) :: buffer
      character(len=1) :: lf
