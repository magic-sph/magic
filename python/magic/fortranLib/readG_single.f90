module greader_single
   ...
   real(kind=4) :: ra,ek,pr,prmag,radratio,sigma
   real(kind=4) :: time
   integer :: nr,nt,np,minc,nric,nThetasBs
   real(kind=4), allocatable :: radius(:),colat(:),radius_ic(:)
   real(kind=4), allocatable :: entropy(:,:,:),vr(:,:,:),vt(:,:,:),vp(:,:,:)
   real(kind=4), allocatable :: Br(:,:,:),Bt(:,:,:),Bp(:,:,:),pre(:,:,:)
   real(kind=4), allocatable :: Br_ic(:,:,:),Bt_ic(:,:,:),Bp_ic(:,:,:)
...
contains
   ...
   subroutine readG(filename, endian)
      ...
      real(kind=4) :: ir,rad,ilat1,ilat2,dumm
      real(kind=4) :: nrF,ntF,npF,mincF,nricF,nThetasBsF
