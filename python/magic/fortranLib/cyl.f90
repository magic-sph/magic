...
subroutine sph_to_cyl(a,ZZ,ac,n_s_max,n_r_max,n_theta_max,r,s,theta)
   ...
   !$omp private(rr0, rr1, rr2, rr3, r10, r20, r30, r21, r31, r32)&
   !$omp private(tt0, tt1, tt2, tt3, t10, t20, t30, t21, t31, t32)&
   !$omp private(a01, a12, a23, a012, a123, itr, ait, n_th, n_hs, n_hs_max)
   sLoop: do n_s=1,n_s_max
      ...
      do n_z=nZstart,nZstop
         ...
         do n_hs=1,n_hs_max
            ...
            if ( n_hs == 1 ) then
               ac(n_z,n_s)=ac(n_z,n_s)+(tt0*a123-tt3*a012)*t30
               ZZ(n_z,n_s)=z
            else ! Index -1 to allow that n_z=0 is not overwritten
               ac(-n_z-1,n_s)=ac(-n_z-1,n_s)+(tt0*a123-tt3*a012)*t30
               ZZ(-n_z-1,n_s)=-z
            end if
         end do
      end do
