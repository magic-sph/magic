# Steps to quickly start using magic
-----------------------------------------------------------------------
...
## 2) Go to the root directory and source the environnement variables (useful
for python and auto-tests)

    `-> cd MAGIC_mpi`

    If you are using sh, bash or zsh as default shell (echo $SHELL), just
...
## 6) Data visualisation and postprocessing
    ...
    a) Set-up your PYTHON environnement (ipython, scipy and matplotlib are needed)
...
## 7) Modify the code and submit your modifications
	...
	4. Never use 'dimension(len)' for declaring array but rather 
           real(cp) :: data(len)
        5. Always use the default precisions when introducing new variables (cp)


    More on that topic on this webpage:

	http://www.fortran90.org/src/best-practices.html

## 8) Make sure you cite the following papers if you intend to publish scientific results
   using Magic:
    * Boussinesq equations: Wicht (2002, PEPI, 132, 281-302)
    * Anelastic equations: Gastine & Wicht (2012, Icarus, 219, 428-442)

    Magic has been tested and validated against several international dynamo
    benchmarks:
        * Christensen et al. (2001, PEPI, 128, 25-34)
        * Jones et al. (2011, Icarus, 216, 120-135)

Contact  gastine [at] mps.mpg.de or wicht [at] mpps.mpg.de in case of any problems.
