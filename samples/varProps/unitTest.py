...
class VariableProperties(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cmd = '{} {}/inputCheb.nml'.format(self.execCmd, self.dir)
        sp.call(cmd, shell=True, stdout=open(os.devnull, 'wb'),
                stderr=open(os.devnull, 'wb'))
        # First run the Chebyshev + Mapping case
        cmd = '{} {}/inputMap.nml'.format(self.execCmd, self.dir)
        sp.call(cmd, shell=True, stdout=open(os.devnull, 'wb'),
                stderr=open(os.devnull, 'wb'))
    ...
    def tearDown(self):
        ...
        cleanDir(self.dir)
        for f in glob.glob('{}/*.cheb'.format(self.dir)):
            os.remove(f)
        for f in glob.glob('{}/*.map'.format(self.dir)):
            os.remove(f)
        for f in glob.glob('{}/*.fd'.format(self.dir)):
            os.remove(f)
