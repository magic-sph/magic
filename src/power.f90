...
!----------------------------------------------------------------------------
   subroutine get_power(time,timePassed,timeNorm,l_stop_time, &
      ...
      real(cp) :: z10ICB,z10CMB,drz10ICB,drz10CMB,powerIC,powerMA
      real(cp) :: powerDiffOld,powerDiffT,dLh
      ...
      do n_r=1,n_r_max
         ...
         !      if ( l == 0 ) cycle
         !      dLh = real(l*(l+1),cp)
         !      laplace=dLh*or2(n_r)*w(lm,n_r)-ddw(lm,n_r)
         !      curlU2_r(n_r)=     curlU2_r(n_r) + dLh * ( &
         !           dLh*or2(n_r)*cc2real(z(lm,n_r),m)  + &
         !           cc2real(dz(lm,n_r),m) + &
         !           cc2real(laplace,m)    )
         !   end do
         !end if
         ...
         if ( l_mag ) then
            ...
            do lm=1,n_mlo_loc
               ...
               if ( l == 0 ) cycle
               dLh = real(l*(l+1),cp)
               laplace=dLh*or2(n_r)*b(lm,n_r)-ddb(lm,n_r)
               curlB2_r(n_r)=curlB2_r(n_r) +  LFfac*opm*eScale*              &
               &             dLh*lambda(n_r)*( dLh*or2(n_r)*                 &
         ...
         if ( l_heat ) then
            ...
            do lm=1,n_mlo_loc
               ...
               if ( l == 0 ) cycle
               dLh = real(l*(l+1),cp)
               buoy_r(n_r)=buoy_r(n_r) + eScale*dLh*BuoFac*          &
         ...
         if ( l_chemical_conv ) then
            ...
            do lm=1,n_mlo_loc
               ...
               if ( l == 0 ) cycle
               dLh = real(l*(l+1),cp)
               buoy_chem_r(n_r)=buoy_chem_r(n_r) + eScale*dLh*ChemFac*    &
      ...
      if ( l_cond_ic .and. l_mag ) then
         ...
         do n_r=1,n_r_ic_max
            ...
            do lm=1,n_mlo_loc
               ...
               &               dLh*r_ratio**(2*l+2) *  ( dLh*O_r_ic2(n_r)*&
