...
SUBROUTINE getDVptr(wS,dwS,ddwS,zS,dzS,rMin,rMax,rS, &
  ...
  INTEGER,INTENT(IN) ::  nZmax,nZmaxA ! number of (r,theta) point to be transformed
  ...
  COMPLEX(kind=8),INTENT(OUT) :: VrS(ncpGeos,nZmaxA)
  COMPLEX(kind=8),INTENT(OUT) :: VtS(ncpGeos,nZmaxA)
  COMPLEX(kind=8),INTENT(OUT) :: VpS(ncpGeos,nZmaxA)
  COMPLEX(kind=8),INTENT(OUT) :: VorS(ncpGeos,nZmaxA)
