...
!------------------------------------------------------------------------------
   subroutine radialLoop(this,l_graph,l_frame,time,timeStage,tscheme,dtLast,  &
      ...
      do nR=nRstart,nRstop
         ...
         if (l_tidal .and. (.not. l_tidal_nl) ) then !ARS: To take into account tidal flow
            this%gsa%vrc(:,:) = this%gsa%vrc(:,:) +vrtidal(:,:,nR)
         ...
         if (l_MRICalc) then !Calc for MRI output
            call get_MRI(this%gsa%vrc,this%gsa%vtc,this%gsa%vpc,    &
                 ...
                 &       this%gsa%dvpdrc,this%gsa%dvrdtc,           &
                 &       this%gsa%dvrdpc,nR)
         end if
...
!-------------------------------------------------------------------------------
   subroutine transform_to_grid_space(this, nR, nBc, lViscBcCalc, lRmsCalc,      &
      ...
      if ( l_conv .or. l_mag_kin ) then
         ...
         if ( nBc == 0 .or. (nBc ==2 .and. l_force_v) ) then ! Bulk points or BC in case of forcing
            !-- pol, sph, tor > ur,ut,up
            call torpol_to_spat(workw, workdw,  work1, &
                 &              this%gsa%vrc, this%gsa%vtc, this%gsa%vpc, l_R(nR))

            !-- Advection is treated as u \times \curl u
            if ( l_adv_curl ) then
               !-- z,dz,w,dd< -> wr,wt,wp
               call torpol_to_curl_spat(or2(nR), workw,&
                    &                      workddw, &
                    &                   work1, work2,              &
               ...
               &    .or. ( l_frame .and. l_movie_oc .and. l_store_frame) ) then
                  call torpol_to_spat(workdw, workddw,         &
                       &              work2, this%gsa%dvrdrc,        &
                       &              this%gsa%dvtdrc, this%gsa%dvpdrc, l_R(nR))
                  call pol_to_grad_spat(workw, this%gsa%dvrdtc, &
                       &                this%gsa%dvrdpc, l_R(nR))
                  call torpol_to_dphspat(workdw,  work1, &
                       &                 this%gsa%dvtdpc, this%gsa%dvpdpc, l_R(nR))
               end if

            else ! Advection is treated as u\grad u

               call torpol_to_spat(workdw, &
                 &                 workddw, work2, &
                 &                 this%gsa%dvrdrc, this%gsa%dvtdrc,             &
                 &                 this%gsa%dvpdrc, l_R(nR))

               call pol_to_curlr_spat(work1, this%gsa%cvrc, l_R(nR))

               call pol_to_grad_spat(workw, &
                    &                this%gsa%dvrdtc, this%gsa%dvrdpc,&
                    &                l_R(nR))
               call torpol_to_dphspat(workdw, &
                    &                 work1, &
                    &                 this%gsa%dvtdpc, this%gsa%dvpdpc, l_R(nR))
            end if
            ...
            if ( lDeriv ) then
               ...
               call torpol_to_spat(workdw, workddw, work2, &
                    &              this%gsa%dvrdrc, this%gsa%dvtdrc,             &
                    &              this%gsa%dvpdrc, l_R(nR))
               call pol_to_curlr_spat(work1, this%gsa%cvrc, l_R(nR))
               call torpol_to_dphspat(workdw,  work1, &
            ...
            if ( lDeriv ) then
               call torpol_to_spat(dw_Rloc(:,nR), ddw_Rloc(:,nR), work2, &
                    &              this%gsa%dvrdrc, this%gsa%dvtdrc,             &
                    &              this%gsa%dvpdrc, l_R(nR))
            end if
         else if (nBc == 3) then
            call torpol_to_spat(w_Rloc(:,nR), dw_Rloc(:,nR),  work1, &
                 &              this%gsa%vrc, this%gsa%vtc, this%gsa%vpc, l_R(nR))
            this%gsa%vrc = 0.0_cp
            this%gsa%vtc = 0.0_cp
            if(lDeriv) then
               this%gsa%dvrdtc = 0.0_cp
               this%gsa%dvrdpc = 0.0_cp
               this%gsa%dvpdpc = 0.0_cp
               this%gsa%dvtdpc = 0.0_cp
               call torpol_to_spat(dw_Rloc(:,nR), ddw_Rloc(:,nR), work2, &
                    &              this%gsa%dvrdrc, this%gsa%dvtdrc, this%gsa%dvpdrc, l_R(nR))
               call pol_to_curlr_spat(work1, this%gsa%cvrc, l_R(nR))
            end if
         end if
