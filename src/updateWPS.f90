...
#elif WITH_ACC_GPU
      !$acc enter data create(ps0Mat, ps0Mat_fac, ps0Pivot, wpsMat, wpsMat_fac, wpsPivot)
      !$acc kernels
      ps0Mat = 0.0_cp; ps0Mat_fac = 0.0_cp; ps0Pivot = 0
      wpsMat = 0.0_cp; wpsMat_fac = 0.0_cp; wpsPivot = 0
      !$acc end kernels
...
#elif WITH_ACC_GPU
      !$acc enter data create(workB, workC)
      !$acc kernels
      workB(:,:) = zero; workC(:,:) = zero
      !$acc end kernels
#endif
...
#elif WITH_ACC_GPU
      !$acc enter data create(rhs1, rhs)
      !$acc kernels
      rhs1(:,:,:) = 0.0_cp
      rhs(:) = 0.0_cp
      !$acc end kernels
...
#elif WITH_ACC_GPU
      !$acc exit data delete(ps0Mat, ps0Mat_fac, ps0Pivot, wpsMat, wpsMat_fac, wpsPivot)
#endif
...
#elif WITH_ACC_GPU
      !$acc exit data delete(rhs1, rhs, workB, workC)
#endif
   ...
   subroutine updateWPS(time,w,dw,ddw,z10,dwdt,p,dp,dpdt,s,ds,dsdt,tscheme,lRmsNext)
      ...
      do nLMB2=1,nLMBs2(nLMB)
         ...
         if ( l1 == 0 ) then
            ...
            call gpu_solve_mat(ps0Mat,2*n_r_max,2*n_r_max,ps0Pivot,rhs,handle,devInfo)
            ...
            if ( .not. lWPSmat(l1) ) then
               call get_wpsMat(tscheme, l1, hdif_V(l1), hdif_S(l1), &
                    &          wpsMat(:,:,nLMB2),                   &
...
#elif WITH_ACC_GPU
            !$acc parallel loop private(lm1, m1, nR)
#endif
            do lm=1,sizeLMB2(nLMB2,nLMB)
               ...
               rhs1(2*n_r_max+1,2*lm-1,0)= real(tops(l1,m1))
               rhs1(2*n_r_max+1,2*lm,0)  =aimag(tops(l1,m1))
               rhs1(3*n_r_max,2*lm-1,0)  = real(bots(l1,m1))
               rhs1(3*n_r_max,2*lm,0)    =aimag(bots(l1,m1))
               ...
               do nR=2,n_r_max-1
                  ...
                  rhs1(nR+n_r_max,2*lm-1,0)  = real(dp(lm1,nR))
                  rhs1(nR+n_r_max,2*lm,0)    =aimag(dp(lm1,nR))
                  rhs1(nR+2*n_r_max,2*lm-1,0)= real(ds(lm1,nR))
                  rhs1(nR+2*n_r_max,2*lm,0)  =aimag(ds(lm1,nR))
               end do

               rhs1(:,2*lm-1,0)=rhs1(:,2*lm-1,0)*wpsMat_fac(:,1,nLMB2)
               rhs1(:,2*lm,0)  =rhs1(:,2*lm,0)*wpsMat_fac(:,1,nLMB2)
            end do
#ifdef WITH_OMP_GPU
            !$omp end target teams distribute parallel do
#elif WITH_ACC_GPU
            ...
            call gpu_solve_mat(wpsMat(:,:,nLMB2),3*n_r_max,3*n_r_max, &
                 &             wpsPivot(:,nLMB2),rhs1(:,:,0),2*lm,handle,devInfo)
            ...
            do lm=1,sizeLMB2(nLMB2,nLMB)
               ...
               rhs1(:,2*lm-1,0)=rhs1(:,2*lm-1,0)*wpsMat_fac(:,2,nLMB2)
               rhs1(:,2*lm,0)  =rhs1(:,2*lm,0)*wpsMat_fac(:,2,nLMB2)
               ...
               if ( m1 > 0 ) then
                  do n_r_out=1,rscheme_oc%n_max
                     ...
                     &                    0),rhs1(2*n_r_max+n_r_out,&
                     &                    2*lm,0),cp)
                  end do
               else
                  do n_r_out=1,rscheme_oc%n_max
                     w(lm1,n_r_out)= cmplx(rhs1(n_r_out,2*lm-1,0),0.0_cp,kind=cp)
                     p(lm1,n_r_out)= cmplx(rhs1(n_r_max+n_r_out,2*lm-1,0), &
                                     &     0.0_cp,kind=cp)
                     s(lm1,n_r_out)= cmplx(rhs1(2*n_r_max+n_r_out,2*lm-1,0), &
                                     &     0.0_cp,kind=cp)
                  end do
               end if
            end do
...
#elif WITH_ACC_GPU
      !$acc enter data create(work_Rloc)
      !$acc kernels
      work_Rloc = zero
      !$acc end kernels
#endif
...
#elif WITH_ACC_GPU
         !$acc parallel loop private(l1,m1,Dif,Buo,dL)
#else
...
#elif WITH_ACC_GPU
            !$acc parallel loop private(l1,m1,Dif,Pre,Buo,dL)
#else
...
#ifdef USE_GPU
      call gpu_prepare_mat(wpsMat,3*n_r_max,3*n_r_max,wpsPivot,info,handle,devInfo)
#else
...
!-----------------------------------------------------------------------------
   subroutine get_ps0Mat(tscheme,psMat,psPivot,psMat_fac)
      ...
      if ( l_temperature_diff ) then ! temperature diffusion
         ...
         do nR_out=1,n_r_max
            ...
            do nR=1,n_r_max
               ...
               psMat(nR,nR_out_p)= -wimp_lin*rscheme_oc%rnorm*kappa(nR)*           &
...
#ifdef WITH_OMP_GPU
            !$omp end target teams distribute parallel do
            !$omp target update from(psMat)
#elif WITH_ACC_GPU
            !$acc end parallel
            !$acc update self(psMat)
#endif
      ...
      !---- LU decomposition:
#ifdef USE_GPU
      call gpu_prepare_mat(psMat,2*n_r_max,2*n_r_max,psPivot,info,handle,devInfo)
#else
