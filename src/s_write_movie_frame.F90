...
SUBROUTINE write_movie_frame(n_frame,time, &
  ...
  DO n_movie=1,n_movies
     ...
     !WRITE(*,"(A,4ES22.14)") "movie: movieDip Colat,Lon,Strength,StrengthGeo = ",&
     !     & movieDipColat,movieDipLon,movieDipStrength,movieDipStrengthGeo
     ...
     IF ( .NOT. lStoreMov(n_movie) ) THEN
        IF ( n_type == 99 ) THEN
           ...
           STOP
        ELSE
           !WRITE(*,"(2A)") "Calling write_dtB_frame for ",movie_file(n_movie)
