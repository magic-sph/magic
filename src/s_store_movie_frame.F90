...
! include 'c_output.f'
    ...
    integer :: n_r                ! radial grid point no.
...
!----------------------------------------------------------------------
    ...
    do n_movie=1,n_movies
        ...
        if ( n_surface == -1 ) then ! Earth Surface
            ...
            if ( n_r /= 1 ) goto 100  ! not CMB radius
            ...
            if ( n_r /= n_const ) goto 100  ! not desired radius
...
!----- Fields (block):
    ...
    integer :: n_r              ! No. of radial point
    integer :: n_store_last     ! Start position in frame(*)-1
    integer :: n_field_type     ! Defines field type
    integer :: n_phi_const      ! No. of surface phi
    integer :: n_field_size     ! Size of field
    integer :: n_theta_start    ! Beginning of theta block
    integer :: n_theta_block    ! Size of theta block
