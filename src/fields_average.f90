module fields_average_mod
   ...
   use sht, only: torpol_to_spat_single, scal_to_spat, sht_l_single
      ...
      if ( l_stop_time .or. mod(nAve,10) == 0 ) then
         ...
         if ( .not. l_onset ) then
            ...
            do nR=nRstart,nRstop
               if ( l_mag ) then
                  call torpol_to_spat_single(b_ave_Rloc(:,nR), db_ave_Rloc(:,nR), &
                       &                     aj_ave_Rloc(:,nR), Br, Bt, Bp, l_R(nR))
               end if
               call torpol_to_spat_single(w_ave_Rloc(:,nR), dw_ave_Rloc(:,nR), &
