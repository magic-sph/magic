   ...
   subroutine fields_average(simtime,tscheme,nAve,l_stop_time,        &
      ...
      if ( l_stop_time .or. mod(nAve,10) == 0 ) then
         ...
         do nR=1,n_r_max
            ...
            if ( l_master_rank ) then
               if ( l_mag ) then
                  call SHloc%torpol_to_spat(b_ave_global, db_ave_global, &
                       &              aj_ave_global, Br, Bt, Bp, l_R(nR))
               end if
               call SHloc%torpol_to_spat(w_ave_global, dw_ave_global, &
                    &              z_ave_global, Vr, Vt, Vp, l_R(nR))
               call SHloc%scal_to_spat(p_ave_global, Prer, l_R(nR))
               call SHloc%scal_to_spat(s_ave_global, Sr, l_R(nR))
               if ( l_chemical_conv ) then
                  call SHloc%scal_to_spat(xi_ave_global, Xir, l_R(nR))
               end if
