...
contains
  ...
  SUBROUTINE get_e_mag(time,l_write,l_stop_time,n_e_sets,         &
    ...
    !-- output:
    REAL(kind=8) :: e_p,e_t            ! poloidal, toroidal energy
    REAL(kind=8) :: e_p_as,e_t_as      ! axisymmetric poloidal, toroidal energy
    REAL(kind=8) :: e_p_ic,e_t_ic   
    REAL(kind=8) :: e_p_as_ic,e_t_as_ic
    REAL(kind=8) :: e_p_es,e_t_es,e_es_cmb,e_as_cmb
    REAL(kind=8) :: e_p_eas,e_t_eas,e_eas_cmb
    REAL(kind=8) :: e_cmb
    REAL(kind=8) :: Dip,DipCMB
    ...
    REAL(kind=8) :: els_r(n_r_max)
    REAL(kind=8) :: e_p_as_r(n_r_max)
    REAL(kind=8) :: e_t_as_r(n_r_max)  
    REAL(kind=8) :: e_p_ic_r(n_r_ic_max)
    REAL(kind=8) :: e_t_ic_r(n_r_ic_max)        
    REAL(kind=8) :: e_p_as_ic_r(n_r_ic_max)
    REAL(kind=8) :: e_t_as_ic_r(n_r_ic_max)  
    REAL(kind=8) :: e_p_es_r(n_r_max),e_t_es_r(n_r_max)
    REAL(kind=8) :: e_p_eas_r(n_r_max),e_t_eas_r(n_r_max)
    REAL(kind=8) :: e_dipole_r(n_r_max)
    REAL(kind=8) :: e_dipole_ax_r(n_r_max)
    REAL(kind=8) :: e_dipole
    REAL(kind=8) :: e_dipole_ax
    REAL(kind=8) :: e_dipole_ax_cmb
    REAL(kind=8) :: e_dipole_e
    REAL(kind=8) :: elsAnel
    REAL(kind=8) :: e_geo,e_es_geo,e_as_geo,e_eas_geo
    REAL(kind=8) :: e_p_os,e_p_as_os
    REAL(kind=8) :: e_p_e,e_p_as_e,e_p_e_ratio
    ...
    DO nR=1,n_r_max
       ...
       DO lm=2,lm_max
          ...
          IF ( l.LE.l_geo .AND. nR.EQ.n_r_cmb ) THEN     
             ...
             IF ( MOD(l+m,2).EQ.1 )                                 &
                  &              e_es_geo =e_es_geo +e_p_temp
             IF ( m.EQ.0 )                                          &
                  &              e_as_geo =e_as_geo +e_p_temp
    ...
    !-- Get Values at CMB:
    e_cmb          =e_p_r(n_r_cmb)+e_t_r(n_r_cmb)
    e_dip_cmb      =e_dipole_r(n_r_cmb)
    e_dipole_ax_cmb=e_dipole_ax_r(n_r_cmb)
    e_es_cmb       =e_p_es_r(n_r_cmb)
    e_as_cmb       =e_p_as_r(n_r_cmb)
    e_eas_cmb      =e_p_eas_r(n_r_cmb)

    ! NOTE: n_e_sets=0 prevents averaging
    IF ( n_e_sets.EQ.1 ) THEN
       timeTot=1.D0
       DO nR=1,n_r_max
          e_dipA(nR) =e_dipole_r(nR)
          e_pA(nR)   =e_p_r(nR)
          e_p_asA(nR)=e_p_r(nR)
          e_tA(nR)   =e_t_r(nR)
          e_t_asA(nR)=e_t_r(nR)
    ...
    ELSE IF ( n_e_sets.EQ.2 ) THEN
       ...
       timeTot=2.D0*dt
       DO nR=1,n_r_max
          e_dipA(nR) =dt*(e_dipA(nR) +e_dipole_r(nR))
    ...
    ELSE
       ...
       timeTot=timeTot+dt
       DO nR=1,n_r_max
          e_dipA(nR) =e_dipA(nR) +dt*e_dipole_r(nR)
    ...
    !-- Radial integrals:
    e_p        =rInt_R(e_p_r        ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_t        =rInt_R(e_t_r        ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_p_as     =rInt_R(e_p_as_r     ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_t_as     =rInt_R(e_t_as_r     ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_p_es     =rInt_R(e_p_es_r     ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_t_es     =rInt_R(e_t_es_r     ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_p_eas    =rInt_R(e_p_eas_r    ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_t_eas    =rInt_R(e_t_eas_r    ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_dipole   =rInt_R(e_dipole_r   ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    e_dipole_ax=rInt_R(e_dipole_ax_r,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    elsAnel    =rInt_R(els_r        ,n_r_max,n_r_max,drx,           &
         &                                     i_costf_init,d_costf_init)
    ...
    e_geo          =fac*e_geo
    e_es_geo       =fac*e_es_geo
    e_as_geo       =fac*e_as_geo
    e_eas_geo      =fac*e_eas_geo
    ...
    IF ( l_cond_ic ) THEN 
       ...
       e_p_ic   =rIntIC(e_p_ic_r,n_r_ic_max,dr_fac_ic,              &
            &                      i_costf1_ic_init,d_costf1_ic_init)
       e_t_ic   =rIntIC(e_t_ic_r,n_r_ic_max,dr_fac_ic,              &
            &                      i_costf1_ic_init,d_costf1_ic_init)
       e_p_as_ic=rIntIC(e_p_as_ic_r,n_r_ic_max,dr_fac_ic,           &
            &                      i_costf1_ic_init,d_costf1_ic_init)
       e_t_as_ic=rIntIC(e_t_as_ic_r,n_r_ic_max,dr_fac_ic,           &
            &                      i_costf1_ic_init,d_costf1_ic_init)
    ...
    IF ( n_imp.EQ.1 ) THEN
       ...
       e_p_e     =fac*e_p_e 
       e_p_as_e  =fac*e_p_as_e 
       e_dipole_e=fac*e_dipole_e 
    END IF


    !-- Output of OC and ouside energies:
    IF ( l_write ) THEN
       IF ( l_save_out ) THEN
