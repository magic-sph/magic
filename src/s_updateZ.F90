...
!-- Input/output of scalar fields:
    ...
    COMPLEX(kind=8),intent(INOUT) :: dz(lm_max,n_r_max)
    COMPLEX(kind=8),intent(IN) :: dzdt(lm_max,n_r_max)
    COMPLEX(kind=8),intent(INOUT) :: dzdtLast(lm_max,n_r_max)
    REAL(kind=8) :: omega_ma,d_omega_ma_dtLast
    REAL(kind=8) :: omega_ic,d_omega_ic_dtLast
    REAL(kind=8) :: lorentz_torque_ma,lorentz_torque_maLast
    REAL(kind=8) :: lorentz_torque_ic,lorentz_torque_icLast
!-- Output:
...
!-- local variables:
    ...
    COMPLEX(kind=8) :: rhs1(2*n_r_max,sizeLMB2max) ! RHS for other modes
    ...
    DO nLMB2=1,nLMBs2(nLMB)
        ...
        DO lm=1,sizeLMB2(nLMB2,nLMB)
            ...
            !      Note: no angular momentum correction necessary for this case !
                IF ( .NOT. lZ10mat ) THEN
                    CALL get_z10Mat(dt,l1,hdif_V(lm1), &
                                    z10Mat,z10Pivot)
                    lZ10mat=.TRUE.
                END IF
                IF ( l_SRMA ) THEN
            ...
            !----- This is the normal RHS for the other radial grid points:
                DO nR=2,n_r_max-1
                    rhs(nR)=O_dt*dLh(lm1)*or2(nR)*z(lm1,nR)+ &
                                            w1*dzdt(lm1,nR)+ &
                                            w2*dzdtLast(lm1,nR)
                END DO
            ...
            ELSE IF ( l1 /= 0 ) THEN
                IF ( .NOT. lZmat(l1) ) THEN
                    CALL get_zMat(dt,l1,hdif_V(lm1), &
                ...
                DO nR=2,n_r_max-1
                    rhs1(nR,lmB)=O_dt*dLh(lm1)*or2(nR)*z(lm1,nR)+ &
                                                 w1*dzdt(lm1,nR)+ &
                                             w2*dzdtLast(lm1,nR)
                END DO
            END IF
        ...
        IF ( lmB > 0 )                                 &
            CALL cgeslML(zMat(1,1,l1),n_r_max,n_r_max, &
                         zPivot(1,l1),rhs1,2*n_r_max,lmB)
...
!-- Get derivatives:
    CALL costf1(z,lm_max_real,lmStart_real,lmStop_real, &
                dzdtLast,i_costf_init,d_costf_init)
    CALL get_ddr(z,dz,workA,lm_max_real,lmStart_real,lmStop_real, &
...
!-- Calculate explicit time step part:
    DO nR=n_r_cmb+1,n_r_icb-1
        DO lm1=lmStart_00,lmStop
            ...
            dLh(lm1)*or2(nR)+dbeta(nR)+2.d0*beta(nR)*or1(nR))* &
                                                  z(lm1,nR) )
            dzdtLast(lm1,nR)=dzdt(lm1,nR)-coex*Dif(lm1)
            IF ( lRmsNext ) THEN
                workB(lm1,nR)= &
                     O_dt*dLh(lm1)*or2(nR)*(z(lm1,nR)-workB(lm1,nR))
                IF ( l_RMStest ) &
                     workB(lm1,nR)= workB(lm1,nR)-Dif(lm1)
            END IF
        END DO
        IF ( lRmsNext ) THEN
            CALL hInt2Tor(Dif,nR,lmStart_00,lmStop, &
                 DifTor2hInt(nR,nTh),DifTorAs2hInt(nR,nTh))
            CALL hInt2Tor(workB(1,nR),nR,lmStart_00,lmStop, &
                 dtVTor2hInt(nR,nTh),dtVTorAs2hInt(nR,nTh))
        END IF
    END DO
