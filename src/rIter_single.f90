   ...
   subroutine transform_to_grid_space(this, nR, nBc, lViscBcCalc, lRmsCalc,      &
      ...
      if ( l_conv .or. l_mag_kin ) then
         if ( l_heat ) then
            ...
            if ( lViscBcCalc ) then
               call scal_to_grad_spat(s_Rdist(:,nR), this%gsa%dsdtc, this%gsa%dsdpc, &
                    &                      l_R(nR))
         ...
         if ( lRmsCalc ) call scal_to_grad_spat(p_Rdist(:,nR), this%gsa%dpdtc, &
                                   &                 this%gsa%dpdpc, l_R(nR))

         !-- Pressure
         if ( lPressCalc ) call scal_to_spat(p_Rdist(:,nR), this%gsa%pc, l_R(nR))

         !-- Composition
         if ( l_chemical_conv ) call scal_to_spat(xi_Rdist(:,nR), this%gsa%xic,l_R(nR))

         if ( l_HT .or. lViscBcCalc ) then
            call scal_to_spat(ds_Rdist(:,nR), this%gsa%drsc, l_R(nR))
         endif
         if ( nBc == 0 ) then ! Bulk points
            ...
            if ( l_adv_curl ) then
               ...
               &    ( l_frame .and. l_movie_oc .and. l_store_frame) ) then
                  ...
                  call pol_to_grad_spat(w_Rdist(:,nR),this%gsa%dvrdtc,  &
               ...
               call torpol_to_spat(dw_Rdist(:,nR), ddw_Rdist(:,nR),        &
                    &              dz_Rdist(:,nR), this%gsa%dvrdrc,        &
                    &              this%gsa%dvtdrc, this%gsa%dvpdrc, l_R(nR))
               call pol_to_curlr_spat(z_Rdist(:,nR), this%gsa%cvrc, l_R(nR))
               call pol_to_grad_spat(w_Rdist(:,nR), this%gsa%dvrdtc, &
                    &                this%gsa%dvrdpc, l_R(nR))
               call torpol_to_dphspat(dw_Rdist(:,nR),  z_Rdist(:,nR), &
                    &                 this%gsa%dvtdpc, this%gsa%dvpdpc, l_R(nR))
            ...
            if ( lDeriv ) then
               ...
               call torpol_to_spat(dw_Rdist(:,nR), ddw_Rdist(:,nR), dz_Rdist(:,nR),  &
                    &              this%gsa%dvrdrc, this%gsa%dvtdrc, this%gsa%dvpdrc,&
                    &              l_R(nR))
               call pol_to_curlr_spat(z_Rdist(:,nR), this%gsa%cvrc, l_R(nR))
               call torpol_to_dphspat(dw_Rdist(:,nR),  z_Rdist(:,nR), &
...
!-------------------------------------------------------------------------------
   subroutine transform_to_lm_space(this, nR, lRmsCalc)
      ...
      if ( l_heat ) then
         call spat_to_qst(this%gsa%VSr, this%gsa%VSt, this%gsa%VSp, this%nl_lm%VSrLM,&
      ...
      if ( lRmsCalc ) then
         ...
         if ( l_conv_nl ) then
            call spat_to_sphertor(this%gsa%Advt2, this%gsa%Advp2, this%nl_lm%Advt2LM,&
