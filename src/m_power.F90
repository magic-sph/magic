...
!********************************************************************
    subroutine get_power(time,timePassed,timeNorm,      &
                         l_stop_time,omega_IC,omega_MA, &
...
!  If inner core and mantle are allowed to change their
...
!-- INPUT of variables:
    ...
    REAL(kind=8) :: time,timePassed,timeNorm
...
!-- OUTPUT:
    REAL(kind=8) :: viscDiss,ohmDiss
            
!-- local:
    INTEGER :: n_r,lm,lmP,l,m

    REAL(kind=8) :: r_ratio
    REAL(kind=8) :: curlB2_r(n_r_max),curlB2
    REAL(kind=8) :: curlU2_r(n_r_max),curlU2
    REAL(kind=8) :: buoy_r(n_r_max),buoy
    REAL(kind=8) :: curlB2_rIC(n_r_max),curlB2_IC
...
!---------------------------------------------------------------------
    ...
    DO n_r=1,n_r_max
        ...
        IF ( l_conv ) THEN
            ...
            DO lm=2,lm_max
                m=lm2m(lm)
                laplace=dLh(lm)*or2(n_r)*w(lm,n_r)-ddw(lm,n_r)
                curlU2_r(n_r)=     curlU2_r(n_r) + dLh(lm) * ( &
                      dLh(lm)*or2(n_r)*cc2real(z(lm,n_r),m)  + &
                                       cc2real(dz(lm,n_r),m) + &
                                       cc2real(laplace,m)    )
            END DO
        END IF
        ...
        IF ( l_mag ) THEN
            ...
            DO lm=2,lm_max
                m=lm2m(lm)
                laplace=dLh(lm)*or2(n_r)*b(lm,n_r)-ddb(lm,n_r)
                curlB2_r(n_r)=curlB2_r(n_r) + dLh(lm)*lambda(n_r)*( &
                           dLh(lm)*or2(n_r)*cc2real(aj(lm,n_r),m) + &
                                            cc2real(dj(lm,n_r),m) + &
                                            cc2real(laplace,m)    )
            END DO
        END IF
...
!-- Transform to cheb space:
    IF ( l_conv ) THEN
        curlU2MeanR=curlU2MeanR+timePassed*curlU2_r*eScale
        curlU2=rInt_R(curlU2_r,n_r_max,n_r_max,drx, &
    ...
    END IF
    IF ( l_mag )  THEN
        ohmDissR=ohmDissR+timePassed*curlB2_r*LFfac*opm*eScale
        curlB2=rInt_R(curlB2_r,n_r_max,n_r_max,drx, &
                      i_costf_init,d_costf_init)
    ELSE
        curlB2=0.D0
    END IF
    IF ( l_heat ) THEN
        buoMeanR=buoMeanR+timePassed*buoy_r*eScale
        buoy=rInt_R(buoy_r,n_r_max,n_r_max,drx, &
                    i_costf_init,d_costf_init)
        buoy=eScale*buoy
    ELSE
        buoy=0.D0
...
!-- Inner core:
    ...
    IF ( l_cond_ic .AND. l_mag ) THEN
        ...
        DO n_r=1,n_r_ic_max
            ...
            DO lm=2,lm_max
                ...
                curlB2_rIC(n_r)=curlB2_rIC(n_r) +                   &
                                      dLh(lm)*r_ratio**(2*l+2) *  ( &
                    dLh(lm)*O_r_ic2(n_r)*cc2real(aj_ic(lm,n_r),m) + &
                                                    cc2real(Bh,m) + &
                                         cc2real(laplace,m)       )
            END DO
        ...
        curlB2_IC=rIntIC(curlB2_rIC,n_r_ic_max,dr_fac_ic, &
...
!  correction terms.
    z10ICB  =REAL(z(2,n_r_ICB))
    drz10ICB=REAL(dz(2,n_r_ICB))
    z10CMB  =REAL(z(2,n_r_CMB))
    drz10CMB=REAL(dz(2,n_r_CMB))
    IF ( l_conv ) THEN
        viscDiss= -curlU2
...
!--- Cause the two systems are coupled only the total ohmic dissipation in usefull:
