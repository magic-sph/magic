...
contains
   ...
   subroutine initialize(this, time_scheme, courfac_nml, intfac_nml, alffac_nml)
      ...
      else if ( index(time_scheme, 'SSP222') /= 0 .or. index(time_scheme, 'PR222') /= 0 &
      &         .or. index(time_scheme, 'LS222') /= 0 ) then
      ...
      else if ( index(time_scheme, 'BFR433') /= 0 ) then
         this%time_scheme = 'BFR433'
         this%nimp = 5
         this%nexp = 5
         this%nstages = 5
         this%istage = 1
         ...
         intfac_loc  = 0.3_cp
         this%l_assembly = .false.
      else if ( index(time_scheme, 'BPR442') /= 0 ) then
         this%time_scheme = 'BPR442'
         this%nimp = 4
         this%nexp = 4
         this%nstages = 4
         this%istage = 1
         ...
         intfac_loc  = 0.46_cp
         this%l_assembly = .false.
      else if ( index(time_scheme, 'DP242') /= 0 ) then
         this%time_scheme = 'DP242'
         this%nimp = 4
         this%nexp = 4
         this%nstages = 4
         this%istage = 1
...
!------------------------------------------------------------------------------
   subroutine set_weights(this, lMatNext)
      ...
      select case ( this%time_scheme )
         ...
         case ('BFR433')
            gam =  0.24169426078821_cp
            a52 = 0.25_cp*gam
            a53 =  0.12915286960590_cp
            a54 = half-a52-a53-gam
            this%wimp_lin(1) = gam
            this%butcher_imp(:,:) = reshape([  0.0_cp, 0.0_cp, 0.0_cp, 0.0_cp, 0.0_cp, &
                                    ...
                                    &          0.0_cp,    a52,    a53,    a54,    gam],&
                                    &        [5,5],order=[2,1])
            this%butcher_ass_imp(:) = [0.0_cp, 0.0_cp, 1.0_cp/6.0_cp, 1.0_cp/6.0_cp, &
            &                          2.0_cp/3.0_cp]
            this%butcher_exp(:,:) = reshape([  0.0_cp, 0.0_cp,  0.0_cp, 0.0_cp, 0.0_cp, &
                                    ...
                                    &          0.0_cp, 0.0_cp, 0.25_cp,0.25_cp, 0.0_cp],&
                                    &        [5,5],order=[2,1])
            this%butcher_ass_exp(:) = this%butcher_ass_imp(:)
            this%l_imp_calc_rhs(1)=.false.
            this%l_exp_calc(1)=.true. ! No need to calculte the explicit solve
            this%l_exp_calc(2)=.false. ! No need to calculte the explicit solve
            this%butcher_c(:) = [0.0_cp, 0.0_cp, one, half]
         case ('SSP222')
            ...
            this%l_imp_calc_rhs(1)=.false.
            this%l_exp_calc(1)=.true. ! No need to calculte the explicit solve
            this%butcher_c(:) = [half, half]
         case ('PR222')
            gam = (3.0_cp+3.0_cp*sqrt(6.0_cp))/3.0_cp
            this%wimp_lin(1) = gam
            this%butcher_imp(:,:) = reshape([  0.0_cp,      0.0_cp, 0.0_cp,  &
                                    &          0.0_cp,         gam, 0.0_cp,  &
         ...
         case ('BHR553')
            ...
            gam = 0.57281606248208_cp
            aa31 = 473447115440655855452482357894373.0_cp/ &
            &      1226306256343706154920072735579148.0_cp
            aa32 = 129298766034131882323069978722019.0_cp/ &
            &      1226306256343706154920072735579148.0_cp
            aa41 = 37498105210828143724516848.0_cp/172642583546398006173766007.0_cp
            aa43 = 76283359742561480140804416.0_cp/172642583546398006173766007.0_cp
            aa51 =-3409975860212064612303539855622639333030782744869519.0_cp/ &
            &      5886704102363745137792385361113084313351870216475136.0_cp
            aa52 =-237416352433826978856941795734073.0_cp/ &
            &      554681702576878342891447163499456.0_cp
            aa53 = 4298159710546228783638212411650783228275.0_cp / &
            &      2165398513352098924587211488610407046208.0_cp
            aa54 = 6101865615855760853571922289749.0_cp / &
            &      272863973025878249803640374568448.0_cp
            a31 = 259252258169672523902708425780469319755.0_cp / &
            &     4392887760843243968922388674191715336228.0_cp
            a32 =-172074174703261986564706189586177.0_cp / &
            &     1226306256343706154920072735579148.0_cp
            a41 = 1103202061574553405285863729195740268785131739395559693754.0_cp / &
            &     9879457735937277070641522414590493459028264677925767305837.0_cp
            a42 =-103754520567058969566542556296087324094.0_cp / &
            &     459050363888246734833121482275319954529.0_cp
            a43 = 3863207083069979654596872190377240608602701071947128.0_cp / &
            &     19258690251287609765240683320611425745736762681950551.0_cp
            b1 =-2032971420760927701493589.0_cp/38017147656515384190997416.0_cp
            b2 = 0.0_cp
            b3 = 2197602776651676983265261109643897073447.0_cp / &
            &    945067123279139583549933947379097184164.0_cp
            b4 =-128147215194260398070666826235339.0_cp / &
            &    69468482710687503388562952626424.0_cp
            c3 = 12015769930846.0_cp/24446477850549.0_cp
            c4 = 3532944.0_cp/5360597.0_cp
         ...
         case ('DP242')
            ...
            &           0.0_cp,        0.0_cp, 0.0_cp, 0.0_cp, 0.0_cp,  &
            &           0.0_cp,        0.0_cp, 0.0_cp, 0.0_cp, 0.0_cp,  &
            &           0.0_cp, 1.0_cp/3.0_cp, 0.0_cp, 0.0_cp, 0.0_cp,  &
            &           0.0_cp,           one, 0.0_cp, 0.0_cp, 0.0_cp,  &
            &           0.0_cp,          half, 0.0_cp,   half, 0.0_cp], &
            &          [5,5],order=[2,1])
            this%butcher_ass_exp(:) = [0.0_cp, half, 0.0_cp, half, 0.0_cp]
            this%l_exp_calc(1)=.true. ! No need to calculte the explicit solve
         ...
         case ('BPR442')
            ...
            this%butcher_imp(:,:) = reshape(                                             &
            &       [       0.0_cp,          0.0_cp,          0.0_cp,   0.0_cp, 0.0_cp,  &
            &               0.0_cp,         0.25_cp,          0.0_cp,   0.0_cp, 0.0_cp,  &
            &               0.0_cp,          0.0_cp,         0.25_cp,   0.0_cp, 0.0_cp,  &
            &               0.0_cp,  1.0_cp/24.0_cp, 11.0_cp/24.0_cp,  0.25_cp, 0.0_cp,  &
            &               0.0_cp, 11.0_cp/24.0_cp,   1.0_cp/6.0_cp, 0.125_cp, 0.25_cp],&
            &              [5,5], order=[2,1])
            this%butcher_ass_imp(:) = [ 0.0_cp, 11.0_cp/24.0_cp, 1.0_cp/6.0_cp, 0.125_cp,&
            &                           0.25_cp ]
            this%butcher_exp(:,:) = reshape(                                        &
            &       [        0.0_cp,        0.0_cp,        0.0_cp, 0.0_cp, 0.0_cp,  &
            &               0.25_cp,        0.0_cp,        0.0_cp, 0.0_cp, 0.0_cp,  &
            &               3.25_cp,       -3.0_cp,        0.0_cp, 0.0_cp, 0.0_cp,  &
            &               0.25_cp,        0.0_cp,          half, 0.0_cp, 0.0_cp,  &
            &                0.0_cp, 1.0_cp/3.0_cp, 1.0_cp/6.0_cp,   half, 0.0_cp], &
            &         [5,5], order=[2,1])
            this%butcher_ass_exp(:) = [ 0.0_cp, 1.0_cp/3.0_cp, 1.0_cp/6.0_cp, half]
            this%butcher_c(:) = [0.25_cp, 0.25_cp, 0.75_cp, one]
            this%l_imp_calc_rhs(1)=.false.
         case ('LZ453')
