module geos
   ...
   use truncation, only: n_phi_max, n_theta_max, n_r_max, nRstart, nRstop,    &
       &                 nThetaStart, nThetaStop, n_theta_loc, radial_balance,&
       &                 load, nR_per_rank, getBlocks
   ...
   real(cp), allocatable :: us_Rdist(:,:,:), up_Rdist(:,:,:), uz_Rdist(:,:,:)
   real(cp), allocatable :: us_Ploc(:,:,:), up_Ploc(:,:,:), uz_Ploc(:,:,:)
   real(cp), allocatable :: wz_Rdist(:,:,:), wz_Ploc(:,:,:)
...
contains
   ...
   subroutine initialize_geos()
      ...
      allocate( us_Rdist(nThetaStart:nThetaStop,n_phi_max,nRstart:nRstop) )
      allocate( up_Rdist(nThetaStart:nThetaStop,n_phi_max,nRstart:nRstop) )
      allocate( uz_Rdist(nThetaStart:nThetaStop,n_phi_max,nRstart:nRstop) )
      allocate( wz_Rdist(nThetaStart:nThetaStop,n_phi_max,nRstart:nRstop) )
      bytes_allocated=bytes_allocated+4*n_phi_max*n_theta_loc*(nRstop-nRstart+1)*&
      &               SIZEOF_DEF_REAL
...
!------------------------------------------------------------------------------------
   subroutine finalize_geos()
      ...
      deallocate( us_Rdist, up_Rdist, uz_Rdist, wz_Rdist  )
      deallocate( cyl, h )
...
!------------------------------------------------------------------------------------
   subroutine calcGeos(vr,vt,vp,cvr,dvrdp,dvpdr,nR)
      ...
      real(cp), intent(in) :: dvrdp(nThetaStart:nThetaStop,n_phi_max)
      real(cp), intent(in) :: dvpdr(nThetaStart:nThetaStop,n_phi_max)

      !-- Local variables
      integer :: nPhi, nTheta

      !$omp parallel do default(shared) private(nTheta,nPhi)
      do nPhi=1,n_phi_max
         do nTheta=nThetaStart,nThetaStop
            !nTheta1=n_theta_cal2ord(nTheta)
            !- us=ur*sin(theta)+ut*cos(theta)
            us_Rdist(nTheta,nPhi,nR)=or2(nR)*orho1(nR)*sinTheta(nTheta)*vr(nTheta,nPhi) &
            &                        +or1(nR)*orho1(nR)*cosTheta(nTheta)*               &
            &                        O_sin_theta(nTheta)*vt(nTheta,nPhi)

            !- uz=ur*cos(theta)-ut*sin(theta)
            uz_Rdist(nTheta,nPhi,nR)=or2(nR)*orho1(nR)*cosTheta(nTheta)*vr(nTheta,nPhi) &
            &                        -or1(nR)*orho1(nR)*vt(nTheta,nPhi)

            !- uphi
            up_Rdist(nTheta,nPhi,nR)=or1(nR)*O_sin_theta(nTheta)*orho1(nR)* &
            &                        vp(nTheta,nPhi)

            !-- z-vorticity
            wz_Rdist(nTheta,nPhi,nR)=or1(nR)*orho1(nR)*(                 &
...
!------------------------------------------------------------------------------------
   subroutine outGeos(time,Geos,GeosA,GeosZ,GeosM,GeosNAP,Ekin)
      ...
      real(cp) :: us_Rloc(n_theta_max,n_phi_max,nRstart:nRstop) !@> TODO: remove !
      real(cp) :: up_Rloc(n_theta_max,n_phi_max,nRstart:nRstop)
      real(cp) :: uz_Rloc(n_theta_max,n_phi_max,nRstart:nRstop)
      real(cp) :: wz_Rloc(n_theta_max,n_phi_max,nRstart:nRstop),tmpp(n_theta_max,n_phi_max)
      integer :: n_p, n_s, n_t, n_r, n_t1


      do n_r=nRstart,nRstop
         !@> TODO: one should find a better fix here!
         call gather_f(us_Rdist(:,:,n_r), us_Rloc(:,:,n_r)) 
         call gather_f(up_Rdist(:,:,n_r), up_Rloc(:,:,n_r)) 
         call gather_f(uz_Rdist(:,:,n_r), uz_Rloc(:,:,n_r)) 
         call gather_f(wz_Rdist(:,:,n_r), wz_Rloc(:,:,n_r)) 
      end do
         ...
         do n_r=nRstart,nRstop
            do n_p=1,n_phi_max
               do n_t=1,n_theta_max
                  ...
                  tmpp(n_t1,n_p)=us_Rloc(n_t,n_p,n_r)
               end do
            end do
            us_Rloc(:,:,n_r)=tmpp(:,:)
            do n_p=1,n_phi_max
               do n_t=1,n_theta_max
                  n_t1=n_theta_cal2ord(n_t)
                  tmpp(n_t1,n_p)=up_Rloc(n_t,n_p,n_r)
               end do
            end do
            up_Rloc(:,:,n_r)=tmpp(:,:)
            do n_p=1,n_phi_max
               do n_t=1,n_theta_max
                  n_t1=n_theta_cal2ord(n_t)
                  tmpp(n_t1,n_p)=uz_Rloc(n_t,n_p,n_r)
               end do
            end do
            uz_Rloc(:,:,n_r)=tmpp(:,:)
            do n_p=1,n_phi_max
               do n_t=1,n_theta_max
                  n_t1=n_theta_cal2ord(n_t)
                  tmpp(n_t1,n_p)=wz_Rloc(n_t,n_p,n_r)
               end do
            end do
            wz_Rloc(:,:,n_r)=tmpp(:,:)
         end do
...
!------------------------------------------------------------------------------------
   subroutine transp_R2Phi(arr_Rloc, arr_Ploc)
      ...
      allocate( rcounts(0:n_ranks_r-1), scounts(0:n_ranks_r-1) )
      allocate( rdisp(0:n_ranks_r-1), sdisp(0:n_ranks_r-1) )
      do p=0,n_ranks_r-1
