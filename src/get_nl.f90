...
!----------------------------------------------------------------------------
   subroutine get_nl(this, time, nR, nBc, lRmsCalc)
      ...
      do nPhi=nPhStart,nPhStop
         ...
         if ( l_conv_nl .and. (nBc == 0 .or. lRmsCalc) .or. (nBc ==2 .and. l_force_v ) ) then
         ...
         if ( l_heat_nl .and. nBc == 0 .or. (nBc ==2 .and. l_force_v )) then
            ...
            ! if (l_tidal_nl) then
            !     cphi=cos(2.0D0*phi(nPhi)-w_orbit_th*time)
         ...
         if ( l_mag_nl ) then
            ...
            if ( (nBc == 0 .and. nR>n_r_LCR) .or. (nBc ==2 .and. l_force_v )) then
            ...
            else if ( nBc >= 2 ) then  ! rigid boundary or boundary forcing ( nBc == 3)
