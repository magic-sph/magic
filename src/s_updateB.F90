...
!***********************************************************************
    ...
    workA,workB,w1,coex,dt,time,nLMB,lRmsNext,nTh)
!***********************************************************************
...
!-- Input/output of scalar potentials and time stepping arrays:
    ...
    COMPLEX(kind=8) :: dbdtLast(lm_maxMag,n_r_maxMag)
    COMPLEX(kind=8) :: djdt(lm_maxMag,n_r_maxMag)
    COMPLEX(kind=8) :: djdtLast(lm_maxMag,n_r_maxMag)
    ...
    COMPLEX(kind=8) :: dbdt_icLast(lm_maxMag,n_r_ic_maxMag)
    COMPLEX(kind=8) :: djdt_icLast(lm_maxMag,n_r_ic_maxMag)
...
!-- Input of non-linear magnetic boundary conditions:
    COMPLEX(kind=8) :: b_nl_cmb(*)  ! nonlinear BC for b at CMB
    COMPLEX(kind=8) :: aj_nl_cmb(*) ! nonlinear BC for aj at CMB
    COMPLEX(kind=8) :: aj_nl_icb(*) ! nonlinear BC for dr aj at ICB
...
!-- right hand sides:
    ...
    COMPLEX(kind=8) :: rhs2(2*n_r_max,sizeLMB2max)
        ...
        DO lm=1,sizeLMB2(nLMB2,nLMB)
            ...
            IF ( l1 > 0 ) THEN
                ...
                IF ( .NOT. lBmat(l1) ) THEN
                    CALL get_bMat(dt,l1,hdif_B(lm1), &
            ...
            !         of poloidal and toroidal magnetic potentials:
                IF ( l_cond_ic ) THEN
                    ...
                    IF ( l_b_nl_icb ) THEN
                        rhs2(n_r_max+1,lmB)=aj_nl_icb(lm1)
                    ELSE
                ...
                DO nR=2,n_r_max-1
                    rhs1(nR,lmB)=                          &
                                     ( w1*dbdt(lm1,nR)   + &
                                   w2*dbdtLast(lm1,nR) ) + &
                       O_dt*dLh(lm1)*or2(nR)*b(lm1,nR)
                    rhs2(nR,lmB)=                          &
                                      ( w1*djdt(lm1,nR)  + &
                                   w2*djdtLast(lm1,nR) ) + &
                      O_dt*dLh(lm1)*or2(nR)*aj(lm1,nR)
                END DO
                ...
                IF ( l_cond_ic ) then    ! inner core
                    DO nR=2,n_r_ic_max
                        ...
                        ELSE
                            fac=-omega_ic*or2(n_r_max)*dPhi(lm1)*dLh(lm1)
                            dbdt_ic=fac*b_ic(lm1,nR)
                            djdt_ic=fac*aj_ic(lm1,nR)
                        END IF
                        rhs1(n_r_max+nR,lmB)=            &
                                          ( w1*dbdt_ic + &
                              w2*dbdt_icLast(lm1,nR) ) + &
                           O_dt*dLh(lm1)*or2(n_r_max)  * &
                                        b_ic(lm1,nR)
                        rhs2(n_r_max+nR,lmB)=            &
                                          ( w1*djdt_ic + &
                              w2*djdt_icLast(lm1,nR) ) + &
                           O_dt*dLh(lm1)*or2(n_r_max)  * &
                                       aj_ic(lm1,nR)
...
!-- We are now back in radial space !
    ...
    DO nR=n_r_cmb+1,n_r_icb-1
        DO lm1=lmStart_00,lmStop
            ...
            coex*opm*lambda(nR)*hdif_B(lm1)*dLh(lm1)*or2(nR) * &
                               (                 ddb(lm1,nR) - &
                                  dLh(lm1)*or2(nR)*b(lm1,nR) )
            djdtLast(lm1,nR)=                   djdt(lm1,nR) - &
            coex*opm*lambda(nR)*hdif_B(lm1)*dLh(lm1)*or2(nR) * &
                              (                  ddj(lm1,nR) + &
                                     dLlambda(nR)*dj(lm1,nR) - &
                                 dLh(lm1)*or2(nR)*aj(lm1,nR) )
            IF ( lRmsNext ) THEN
                workA(lm1,nR)=O_dt*dLh(lm1)*or2(nR) * &
        ...
        IF ( lRmsNext ) THEN
            CALL hInt2Pol(workA(1,nR),nR,lmStart_00,lmStop,dtBPolLMr, &
                          dtBPol2hInt(nR,nTh),dtBPolAs2hInt(nR,nTh))
            CALL hInt2Tor(workB(1,nR),nR,lmStart_00,lmStop, &
        ...
        DO nR=2,n_r_ic_max-1
            DO lm1=lmStart_00,lmStop
                dbdt_icLast(lm1,nR)=dbdt_icLast(lm1,nR) - &
                    coex*opm*O_sr*dLh(lm1)*or2(n_r_max) * &
                (                        ddb_ic(lm1,nR) + &
                2.d0*D_lP1(lm1)*O_r_ic(nR)*db_ic(lm1,nR) )
                djdt_icLast(lm1,nR)=djdt_icLast(lm1,nR) - &
                    coex*opm*O_sr*dLh(lm1)*or2(n_r_max) * &
                (                        ddj_ic(lm1,nR) + &
                2.d0*D_lP1(lm1)*O_r_ic(nR)*dj_ic(lm1,nR) )
            END DO
        END DO
        nR=n_r_ic_max
        DO lm1=lmStart_00,lmStop
            dbdt_icLast(lm1,nR)=dbdt_icLast(lm1,nR) - &
                coex*opm*O_sr*dLh(lm1)*or2(n_r_max) * &
              (1.d0+2.d0*D_lP1(lm1))*ddb_ic(lm1,nR)
            djdt_icLast(lm1,nR)=djdt_icLast(lm1,nR) - &
                coex*opm*O_sr*dLh(lm1)*or2(n_r_max) * &
              (1.d0+2.d0*D_lP1(lm1))*ddj_ic(lm1,nR)
        END DO
    END IF
