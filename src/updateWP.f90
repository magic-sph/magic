module updateWP_mod
   ...
   use fields, only: work_LMloc, s_Rloc, xi_Rloc, wtidal_LMLoc, dwtidal_LMloc, ddwtidal_LMloc
...
!-----------------------------------------------------------------------------
   subroutine updateWP(time, s, xi, w, dw, ddw, dwdt, p, dp, dpdt, tscheme, &
      ...
      integer :: nLMB
      real(cp):: l2m2, fac_stress !for tidal and ktopv=1
            ...
            do iChunk=1,nChunks
               ...
               do lm=lmB0+1,min(iChunk*chunksize,sizeLMB2(nLMB2,nLMB))
                  ...
                  if ( l_double_curl ) then
                     ...
                     ! if (l_tidal) then
                     !    if (ktopv==1) then
                     !       fac_stress=-two*or1(1)-beta(1)
                     !       rhs1(n_r_max+1,2*lm-1,threadid)= &
                     !            & real(-ddwtidal_LMloc(lm,1)-fac_stress*dwtidal_LMloc(lm,1))
                     !       rhs1(n_r_max+1,2*lm,threadid)=  & 
                     !            & aimag(-ddwtidal_LMloc(lm,1)-fac_stress*dwtidal_LMloc(lm,1))
                     !    end if
                     !    if (kbotv==1) then
                     !       fac_stress=-two*or1(n_r_max)-beta(n_r_max)
                     !       rhs1(2*n_r_max,2*lm-1,threadid)= &
                     !        & real(-ddwtidal_LMloc(lm,n_r_max)-fac_stress*dwtidal_LMloc(lm,n_r_max))
                     !       rhs1(2*n_r_max,2*lm,threadid)= &
                     !            & aimag(-ddwtidal_LMloc(lm,n_r_max)-fac_stress*dwtidal_LMloc(lm,n_r_max))
                     !    end if
                     ! else
                     rhs1(n_r_max+1,2*lm-1,threadid)=0.0_cp
                     rhs1(n_r_max+1,2*lm,threadid)  =0.0_cp
                  ...
                  if ( l_double_curl ) then
                     if ( m1 > 0 ) then
                        ...
                        if (L_force_v .and. force_z_vol == 2) then
                           do n_r_out=1,rscheme_oc%n_max
                              w(lm1,n_r_out)  = cmplx(0.0_cp,0.0_cp,kind=cp)
                           end do
                        else
                           do n_r_out=1,rscheme_oc%n_max
                              w(lm1,n_r_out)  = cmplx(rhs1(n_r_out,2*lm-1,threadid),&
...
!------------------------------------------------------------------------------
   subroutine assemble_pol(s, xi, w, dw, ddw, p, dp, dwdt, dpdt, dp_expl, &
      ...
      !-- Local variables
      complex(cp) :: Dif, Buo, rhs_top, rhs_bot
         ...
         if ( l_full_sphere ) then
            if ( ktopv == 1 ) then ! Stress-free
               ...
               do lm=start_lm,stop_lm
                  ...
                  !    rhs_top = -one*ddwtidal_LMloc(lm,1)-fac_top*dwtidal_LMloc(lm,1)
                  !    call rscheme_oc%robin_bc(one, fac_top, rhs_top, one, 0.0_cp, zero, dw(lm,:))
                  else
            ...
            if ( ktopv /= 1 .and. kbotv /= 1 ) then ! Rigid at both boundaries
               ...
               ! if (l_tidal) then
               !    rhs_bot = -one*ddwtidal_LMloc(lm,n_r_max)-fac_top*dwtidal_LMloc(lm,n_r_max)
               ! else
               rhs_bot=zero
               ! end if
               fac_bot=-two*or1(n_r_max)-beta(n_r_max)
               do lm=start_lm,stop_lm
                  call rscheme_oc%robin_bc(0.0_cp, one, zero, one, fac_bot, rhs_bot, dw(lm,:))
               end do   
               ...
               fac_top=-two*or1(1)-beta(1)
               do lm=start_lm,stop_lm
                  call rscheme_oc%robin_bc(one, fac_top, rhs_top, one, fac_bot, rhs_bot, dw(lm,:))
               end do
            end if
