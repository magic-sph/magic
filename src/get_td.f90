module nonlinear_lm_mod
   ...
   use logic, only : l_anel, l_conv_nl, l_corr, l_heat, l_anelastic_liquid, &
       ...
       &             l_tidal
   use radial_functions, only: r, or2, or1, beta, epscProf, or4, temp0, orho1, l_R, rho0
   use physical_parameters, only: CorFac, epsc,  n_r_LCR, epscXi, tidalFac, w_orbit_th
...
!----------------------------------------------------------------------------
   subroutine get_td(this,nR,nBc,lPressNext,dVSrLM,dVXirLM,dVxVhLM,dVxBhLM, &
              &      dwdt,dzdt,dpdt,dsdt,dxidt,dbdt,djdt,time)
      !
      ...
      real(cp), intent(in):: time !for Tidal force
      ...
      if (nBc == 0 ) then
         ...
         if ( l_conv ) then  ! Convection
            ...
            if ( l_corr ) then
               CorPol_loc=two*CorFac*or1(nR) * dTheta2A(lm)* (z_Rloc(lmA,nR)+z0v_Rloc(lmA,nR))
