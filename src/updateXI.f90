...
module updateXi_mod
   ...
   use omp_lib
   use precision_mod
   use truncation, only: n_r_max, get_openmp_blocks, n_mlo_loc, n_lo_loc, &
...
!------------------------------------------------------------------------------
   subroutine updateXi(xi, dxi, dxidt, tscheme)
      ...
      complex(cp),       intent(inout) :: xi(n_mlo_loc,n_r_max) ! Chemical composition
      type(type_tarray), intent(inout) :: dxidt
      !-- Output: dxi
      complex(cp),       intent(out) :: dxi(n_mlo_loc,n_r_max) ! Radial derivative of xi
...
!------------------------------------------------------------------------------
   subroutine finish_exp_comp_Rdist(dVXirLM, dxi_exp_last)
      ...
      !-- Input variables
      complex(cp), intent(inout) :: dVXirLM(n_lm_loc,nRstart:nRstop)

      !-- Output variables
      complex(cp), intent(inout) :: dxi_exp_last(n_lm_loc,nRstart:nRstop)

      !-- Local variables
      complex(cp) :: work_Rloc(n_lm_loc,nRstart:nRstop)
      integer :: n_r

      call get_dr_Rloc(dVXirLM, work_Rloc, n_lm_loc, nRstart, nRstop, n_r_max, &
           &           rscheme_oc)
...
!------------------------------------------------------------------------------
   subroutine assemble_comp(xi, dxi, dxidt, tscheme)
      ...
      call tscheme%assemble_imex(work_LMdist, dxidt, 1, n_mlo_loc, n_r_max)
      ...
      do n_r=2,n_r_max
         do lm=1,n_mlo_loc
            m = map_mlo%i2m(lm)
            if ( m == 0 ) then
               xi(lm,n_r)=cmplx(real(work_LMdist(lm,n_r)),0.0_cp,cp)
            else
               xi(lm,n_r)=work_LMdist(lm,n_r)
            end if
         end do
      end do
      !$omp end do
      ...
      if ( l_full_sphere) then
         if ( ktopxi == 1 ) then ! Fixed entropy at the outer boundary
            ...
            do lm=1,n_mlo_loc
               ...
               m = map_mlo%i2m(lm)
               if ( l == 1 ) then
                  call rscheme_oc%robin_bc(0.0_cp, one, topxi(l,m), 0.0_cp, one, &
                       &                   botxi(l,m), xi(lm,:))
               else
                  call rscheme_oc%robin_bc(0.0_cp, one, topxi(l,m), one, 0.0_cp, &
                       &                   botxi(l,m), xi(lm,:))
               end if
            end do
            !$omp end do
            ...
            do lm=1,n_mlo_loc
               ...
               m = map_mlo%i2m(lm)
               if ( l == 1 ) then
                  call rscheme_oc%robin_bc(one, 0.0_cp, topxi(l,m), 0.0_cp, one, &
                       &                   botxi(l,m), xi(lm,:))
               else
                  call rscheme_oc%robin_bc(one, 0.0_cp, topxi(l,m), one, 0.0_cp, &
                       &                   botxi(l,m), xi(lm,:))
               end if
            end do
            !$omp end do
