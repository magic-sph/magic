...
#elif WITH_ACC_GPU
         !$acc enter data create(bMat_fac, jMat_fac)
         !$acc kernels
         bMat_fac(:,:) = 0.0_cp
         jMat_fac(:,:) = 0.0_cp
         !$acc end kernels
...
#ifdef WITH_OMP_GPU
            !$omp target enter data map(alloc: workA, workB)
#elif WITH_ACC_GPU
            !$acc enter data create(workA, workB)
#endif
...
#elif WITH_ACC_GPU
         !$acc enter data create (rhs1, rhs2)
         !$acc kernels
         rhs1 = 0.0_cp; rhs2 = 0.0_cp
         !$acc end kernels
#endif
...
#elif WITH_ACC_GPU
         !$acc enter data create(b_ghost, aj_ghost)
         !$acc kernels
         b_ghost(:,:) =zero
         aj_ghost(:,:)=zero
         !$acc end kernels       
...
#elif WITH_ACC_GPU
         !$acc enter data create(tmpb_LMloc, tmpj_LMloc)
         !$acc kernels
         tmpb_LMloc(:,:)=zero
         tmpj_LMloc(:,:)=zero
         !$acc end kernels
...
#elif WITH_ACC_GPU
      !$acc enter data create(datJmat, datBmat)
      !$acc kernels
      datJmat = 0.0_cp
      datBmat = 0.0_cp
      !$acc end kernels
...
!-----------------------------------------------------------------------------
   subroutine finalize_updateB()
      ...
      if ( l_RMS ) then 
#ifdef WITH_OMP_GPU
            !$omp target exit data map(delete: workA, workB)
#elif WITH_ACC_GPU
            !$acc exit data delete(workA, workB)
#endif
...
#elif WITH_ACC_GPU
         !$acc exit data delete(bMat_fac, jMat_fac)
#endif
...
#elif WITH_ACC_GPU
         !$acc exit data delete(rhs1, rhs2)
#endif
...
#elif WITH_ACC_GPU
         !$acc exit data delete(datJmat, datBmat)
#endif
...
#elif WITH_ACC_GPU
         !$acc exit data delete(tmpb_LMloc, tmpj_LMloc)
#endif
...
#elif WITH_ACC_GPU
         !$acc enter data copyin(aj_nl_icb)
#endif
      end if
      if ( l_b_nl_cmb ) then
#ifdef WITH_OMP_GPU
...
#elif WITH_ACC_GPU
         !$acc enter data copyin(aj_nl_cmb, b_nl_cmb)
#endif
      end if
...
#elif WITH_ACC_GPU
      !$acc parallel loop private(l1, m1, yl0_norm, prefac,bpeaktop, ff, aimp, cimp)
#endif
      do lm=llmMag,ulmMag
         l1=lm2l(lm)
         m1=lm2m(lm)
...
#elif WITH_ACC_GPU
         !$acc parallel loop private(l1,m1)
#endif
         do lm=llmMag,ulmMag
            l1=lm2l(lm)
            m1=lm2m(lm)
...
#elif WITH_ACC_GPU
      !$acc parallel loop collapse(2) private(l1,nLMB2)
#endif
      do lm=llmMag,ulmMag
         do nR=1,n_r_tot
            l1=lm2l(lm)
...
#elif WITH_ACC_GPU
      !$acc parallel loop collapse(2) private(l1,m1)
#endif
      do lm=llmMag,ulmMag
...
#elif WITH_ACC_GPU
      !$acc parallel loop private(lm1,m1,nR,ff,cimp,aimp)
#endif
...
#elif WITH_ACC_GPU
            !$acc parallel loop private(lm1, m1, n_r_out)
#endif
            do lm=1,sizeLMB2(nLMB2,nLMB)
               lm1=lm22lm(lm,nLMB2,nLMB)
               m1=lm22m(lm,nLMB2,nLMB)
...
#elif WITH_ACC_GPU
            !$acc update self(b_ic, aj_ic, dbdt_ic, djdt_ic)
#endif
...
#elif WITH_ACC_GPU
         !$acc exit data delete(aj_nl_icb)
#endif
      end if
      if ( l_b_nl_cmb ) then
#ifdef WITH_OMP_GPU
...
#elif WITH_ACC_GPU
         !$acc exit data delete(b_nl_cmb, aj_nl_cmb)
#endif
      end if
#endif
...
#elif WITH_ACC_GPU
         !$acc parallel loop collapse(2) private(fac,l1,m1)
#else
...
#elif WITH_ACC_GPU
      !$acc update device(b_ic, db_ic, ddb_ic, aj_ic, dj_ic, ddj_ic)
#endif
...
#elif WITH_ACC_GPU
         !$acc update self(b, aj)
#endif
         if ( l_cond_ic ) then
#ifdef WITH_OMP_GPU
            !$omp target update from(b_ic, aj_ic)
#elif WITH_ACC_GPU
            !$acc update self(b_ic, aj_ic)
#endif
         end if
...
#elif WITH_ACC_GPU
         !$acc update device(b, aj)
#endif
         if ( l_cond_ic ) then
#ifdef WITH_OMP_GPU
            !$omp target update to(b_ic, aj_ic)
#elif WITH_ACC_GPU
         !$acc update device(b_ic, aj_ic)
#endif
         end if
...
#elif WITH_ACC_GPU
         !$acc parallel loop collapse(2) private(l1,m1,dL,dtP,dtT)
#else
...
#elif WITH_ACC_GPU
         !$acc parallel loop collapse(2) private(l,m,dtP,dtT,dL) 
#endif
         do n_r=nRstartMag,nRstopMag
            do lm=start_lm,stop_lm
               l=lm2l(lm)
               ...
               if ( l > 0 ) then
                  ...
                  if ( lRmsNext .and. loc_istage == loc_nstage ) then
                     dtP=dL*or2(n_r)/loc_dt * (  bg(lm,n_r)-workA(lm,n_r) )
                     dtT=dL*or2(n_r)/loc_dt * ( ajg(lm,n_r)-workB(lm,n_r) )
...
!-----------------------------------------------------------------------------
   ...
   subroutine get_bMat(tscheme,l,hdif,bMat,jMat,nLMB2,l_LU)
...
#elif WITH_ACC_GPU
      !$acc update self(datJmat, datBmat)
#endif
      ...
      !$omp target update to(datJmat, datBmat)
#elif WITH_ACC_GPU
      !$acc update device(datJmat, datBmat)
#endif
   ...
   subroutine get_bMat_Rdist(tscheme,hdif,bMat,jMat)
      ...
      do nR=1,n_r_max
         do l=1,l_maxMag
            ...
            bMat%diag(l,nR)=dLh*or2(nR) - wimp_lin*opm*lambda(nR)* &
            &               hdif(l)*dLh*or2(nR) * (    rscheme_oc%ddr(nR,1) - &
            &                      dLh*or2(nR) )
            bMat%low(l,nR)=-wimp_lin*opm*lambda(nR)*hdif(l)*dLh*   &
            &              or2(nR)*rscheme_oc%ddr(nR,0)
            bMat%up(l,nR) =-wimp_lin*opm*lambda(nR)*hdif(l)*dLh*   &
            &              or2(nR)*rscheme_oc%ddr(nR,2)

            jMat%diag(l,nR)=dLh*or2(nR) - wimp_lin*opm*lambda(nR)* &
            &               hdif(l)*dLh*or2(nR) * ( rscheme_oc%ddr(nR,1) +    &
            &               dLlambda(nR)*rscheme_oc%dr(nR,1) - dLh*or2(nR) )
            jMat%low(l,nR)=-wimp_lin*opm*lambda(nR)*hdif(l)*dLh*   &
            &      or2(nR)*(rscheme_oc%ddr(nR,0)+dLlambda(nR)*rscheme_oc%dr(nR,0))
            jMat%up(l,nR) =-wimp_lin*opm*lambda(nR)*hdif(l)*dLh*   &
...
#elif WITH_ACC_GPU
      !$acc update self(bMat%diag, bMat%up, bMat%low, jMat%diag, jMat%up, jMat%low)
#else
      !$omp end parallel
#endif
