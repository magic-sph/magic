...
module output_mod
   ...
   use logic, only: l_average, l_mag, l_power, l_anel, l_mag_LF, lVerbose,    &
       ...
       &            l_dtPhaseMovie, l_geosMovie, l_MRI, l_gw, l_mag_hel
   ...
   use outMisc_mod, only: outHeat, outHelicity, outHemi, outPhase, get_onset, &
       &                  calc_melt_frame, outGWentropy, outGWpressure !, outMagneticHelicity !PNS
      ...
      if ( l_log ) then
         ...
         if ( l_power ) then
            ...
            call get_power( time,timePassedLog,timeNormLog,l_stop_time,      &
                 ...
                 &          visDiss,ohmDiss,dw_LMloc,ddw_LMloc)
            PERFOFF
...
!                 &        GeosNAP,dpV,dzV,volume,EC)
      ...
      if ( l_MRI .and. l_log) then
         if (lVerbose) write (output_unit,*) '!Calling outMRI'

         call outMRI(time,n_time_step,nMRIsets)

         if ( lVerbose ) write(output_unit,*) '! outMRI finished !'
         if (DEBUG_OUTPUT) write(output_unit,"(A,I6)") "Written  MRI  on rank ",rank
      end if
