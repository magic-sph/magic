module updateS_mod
   ...
   use physical_parameters, only: opr, kbots, ktops, stef, w_orbit_th
...
!------------------------------------------------------------------------------
   subroutine finish_exp_entropy(w, dVSrLM, ds_exp_last,time)
      !
      ...
      !-- Local variables
      real(cp) :: dL
      complex(cp) :: eiwt(llm:ulm,n_r_max)
      integer :: n_r, lm, start_lm, stop_lm, l, m
...
!-----------------------------------------------------------------------------
   subroutine finish_exp_entropy_Rdist(w, dVSrLM, ds_exp_last,time)
      ...
      if (l_tidal) then
         eiwt=cmplx(cos(-w_orbit_th*time),sin(-w_orbit_th*time),kind=cp)
      else
         eiwt=0.0_cp
      end if
      ...
      if ( l_anelastic_liquid ) then
         do n_r=nRstart,nRstop
            do lm=start_lm,stop_lm
               ...
               &       or2(n_r)*orho1(n_r)*dLtemp0(n_r)*dVSrLM(lm,n_r) - &
               &        dL*or2(n_r)*orho1(n_r)*temp0(n_r)*dentropy0(n_r)*&
               &        (w(lm,n_r)+wtidal_LMloc(lm,n_r)*eiwt)
            end do
         end do
      else
         do n_r=nRstart,nRstop
