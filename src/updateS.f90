...
#elif WITH_ACC_GPU
         !$acc enter data create(sMat_fac)
         !$acc kernels
         sMat_fac(:,:) = 0.0_cp
         !$acc end kernels
#endif
...
#elif WITH_ACC_GPU
         !$acc enter data create(fd_fac_bot, fd_fac_top)
         !$acc kernels
         fd_fac_top(:)=0.0_cp
         fd_fac_bot(:)=0.0_cp         
         !$acc end kernels
...
#elif WITH_ACC_GPU
         !$acc exit data delete(s_ghost, fd_fac_top, fd_fac_bot)
#endif
...
!------------------------------------------------------------------------------
   subroutine updateS(s, ds, dsdt, phi, tscheme)
      ...
      integer :: info           ! Integer to validate LU factorisation
...
#elif WITH_ACC_GPU
      !$acc parallel loop collapse(2) private(m1)
#endif
      do lm=llm,ulm
...
#elif WITH_ACC_GPU
         !$acc parallel loop private(lm1, l1, m1, nR)
#endif
   ...
   subroutine get_entropy_rhs_imp(s, ds, dsdt, phi, istage, l_calc_lin, l_in_cheb_space)
      ...
      if ( l_calc_lin ) then
         ...
         if ( l_anelastic_liquid ) then
            ...
            !$omp target teams distribute parallel do collapse(2)
#elif WITH_ACC_GPU
            !$acc parallel loop collapse(2) private(l1,dL)
#else
            !$omp do private(n_r,lm,l1,dL)
#endif
            do n_r=1,n_r_max
...
!-------------------------------------------------------------------------------
#ifdef WITH_PRECOND_S
   subroutine get_sMat(tscheme,l,hdif,sMat,nLMB2,sMat_fac,l_LU)
#else
   subroutine get_sMat(tscheme,l,hdif,sMat,nLMB2,l_LU)
#endif
      !
