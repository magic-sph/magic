   ...
   subroutine readStartFields_old(w,dwdt,z,dzdt,p,dpdt,s,dsdt,xi,dxidt,phi,   &
      ...
      if ( tscheme%family == 'MULTISTEP' .and. tscheme%nexp >=2 ) then
         ...
         if ( l_single_matrix ) then
            ...
            !$acc update device(s)
            !$acc update if(l_chemical_conv) device(xi)
            !$acc update device(z)
#endif
...
#elif WITH_ACC_GPU
               !$acc update self(s, ds_LMloc, dsdt)
#endif
            end if
         end if
