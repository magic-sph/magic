module updateZ_mod
   ...
   use radial_functions, only: visc, or1, or2, rscheme_oc, dLvisc, beta, &
       &                       rho0, r_icb, r_cmb, r, dbeta, drag0
   ...
   use logic, only: l_rot_ma, l_rot_ic, l_SRMA, l_SRIC, l_z10mat, l_precession, &
       ...
       &            l_finite_diff, l_full_sphere, l_drag, l_force_v
   ...
   subroutine initialize_updateZ
      ...
      if ( .not. l_parallel_solve ) then
         ...
         if ( l_finite_diff ) then
            ...
            allocate( type_bandmat :: zm0Mat(nLMBs2(1+rank)) )
            ...
            do ll=1,nLMBs2(1+rank)
               ...
               call zm0Mat(ll)%initialize(n_bands,n_r_max,l_pivot=.true.)
            end do
            ...
            allocate( type_densemat :: zm0Mat(nLMBs2(1+rank)) )
         ...
         do ll=1,nLMBs2(1+rank)
            ...
            call zm0Mat(ll)%initialize(n_r_max,n_r_max,l_pivot=.true.)
         end do
      end if
         ...
         allocate(zm0Mat_fac(n_r_max,nLMBs2(1+rank)))
         bytes_allocated = bytes_allocated+2*n_r_max*nLMBs2(1+rank)*SIZEOF_DEF_REAL
#endif
...
!-------------------------------------------------------------------------------
   subroutine updateZ(time,timeNext,z,dz,dzdt,omega_ma,omega_ic,domega_ma_dt,  &
      ...
      !-- local variables:
      integer :: l1,l,m1              ! degree and order
      ...
      do nLMB2=1,nLMBs2(nLMB)
         ...
         if ( l1 /= 0 ) then
            if ( .not. lZmat(l1) ) then
               ...
               if (l_force_v .and. force_z_vol == 1) then
                  call get_zm0Mat(tscheme,l1,hdif_V(l1),zMat(nLMB2),zm0Mat_fac(:,nLMB2))
               end if
#else
               call get_zMat(tscheme,l1,hdif_V(l1),zMat(nLMB2))
               if (l_force_v  .and. force_z_vol == 1) then
                  call get_zm0Mat(tscheme,l1,hdif_V(l1),zMat(nLMB2))
               end if
#endif
               lZmat(l1)=.true.
            end if
            ...
            do iChunk=1,nChunks
               ...
               do lm=lmB0+1,min(iChunk*chunksize,sizeLMB2(nLMB2,nLMB))
                  ...
                  if ( l_z10mat .and. lm1 == l1m0 ) then
                     ...
                     if ( l_SRIC ) then
                        if(kbotv==3) then
                           rhs(n_r_max) = real(botv(l1,m1))!/c_z10_omega_ic
                        else
                     ...
                     rhs(:) = z10Mat_fac(:)*rhs(:)
#endif
                     if (l_force_v .and. force_z_vol == 2) then
                        rhs(:)=zero
                     else
                     ...
                     if (ktopv == 3) then
                        rhs1(1,2*lmB-1,threadid)      =real(topv(l1,m1))
                        rhs1(1,2*lmB,threadid)        =aimag(topv(l1,m1))
                     else
                        rhs1(1,2*lmB-1,threadid)      =0.0_cp
                        rhs1(1,2*lmB,threadid)        =0.0_cp
                     end if
                     if (kbotv == 3) then
                        rhs1(n_r_max,2*lmB-1,threadid)      =real(botv(l1,m1))
                        rhs1(n_r_max,2*lmB,threadid)        =aimag(botv(l1,m1))
                     else
...
#ifdef WITH_PRECOND_Z
                     if (l_force_v .and. force_z_vol == 1 .and. m1 == 0) then
                        rhs1(:,2*lmB-1,threadid)=zm0Mat_fac(:,nLMB2)*rhs1(:,2*lmB-1,threadid)
                        rhs1(:,2*lmB,threadid)  =zm0Mat_fac(:,nLMB2)*rhs1(:,2*lmB,threadid)
                     else
               ...
               !-- Linear solves
               if (l_force_v .and. force_z_vol == 1 .and. m1 == 0) then
                  call zm0Mat(nLMB2)%solve(rhs1(:,2*(lmB0+1)-1:2*lmB,threadid),2*(lmB-lmB0))
               else
               ...
               do lm=lmB0+1,min(iChunk*chunksize,sizeLMB2(nLMB2,nLMB))
                  ...
                  if ( l_z10mat .and. lm1 == l1m0 ) then
                     ...
                     if ( m1 > 0 ) then
                        ...
                        if (l_force_v .and. force_z_vol == 2) then
                              z(lm1,:)=cmplx(0.0_cp, 0.0_cp,kind=cp)
                        else
                           do n_r_out=1,rscheme_oc%n_max
                              z(lm1,n_r_out)=cmplx(rhs1(n_r_out,2*lmB-1,threadid), &
                           &                    0.0_cp,kind=cp)
   ...
   subroutine assemble_tor(time, z, dz, dzdt, domega_ic_dt, domega_ma_dt,      &
      ...
      do lm=llm,ulm
         ...
         if ( lm==l1m0 ) then
            ...
            if (ktopv == 3) then
               top_val(lm) = topv(l1,m1)
            else
               top_val(lm)=zero
            end if
            if (kbotv == 3) then
               bot_val(lm)= botv(l1,m1)
            else
               bot_val(lm)=zero
            end if
         end if
      ...
      if ( llm <= l1m0 .and. ulm >= l1m0 )then
         if ( l_rot_ma .and. .not. l_SRMA ) then
            if ( ktopv == 1 ) then  ! free slip, explicit time stepping of omega !
               ...
               &        gammatau_gravi))
            else if ( ktopv >= 2 ) then ! no slip, omega given by z10 !ARS
               z10(:)=real(z(l1m0,:))
   ...
   subroutine get_z10Mat(tscheme,l,hdif,zMat)
      ...
      do nR_out=1,n_r_max
         do nR=2,n_r_max-1
            ...
            &+ tscheme%wimp_lin(1)*(drag0(nR)+tau)*rscheme_oc%rMat(nR,nR_out)*dLh*or2(nR) )
         end do
      end do
      !-- Normalisation
      do nR=1,n_r_max
...
!-------------------------------------------------------------------------------
#ifdef WITH_PRECOND_Z
   subroutine get_zm0Mat(tscheme,l,hdif,zMat,zMat_fac)
#else
   subroutine get_zm0Mat(tscheme,l,hdif,zMat)
#endif
      !
      !  Purpose of this subroutine is to contruct the time step matrices
      !  ``zmat(i,j)`` for the Navier-Stokes equation.
...
!------------------------------------------------------------------------------
   subroutine get_z10Mat_Rdist(tscheme,hdif,zMat)
      ...
      do nR=1,n_r_max
         ...
         &          +dLh*or2(nR)+dbeta(nR)+two*beta(nR)*or1(nR)       &
         &                             ) )                            &
         &  + tscheme%wimp_lin(1)*drag0(nR)*dLh*or2(nR)
