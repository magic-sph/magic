...
module rIterThetaBlocking_OpenMP_mod
   ...
   use num_param, only: phy2lm_counter, lm2phy_counter, nl_counter, &
       &                td_counter
   use truncation, only: lmP_max, n_r_cmb, n_r_icb, lmP_max_dtB
...
!------------------------------------------------------------------------------
   subroutine initialize_rIterThetaBlocking_OpenMP(this)
      ...
      call this%dtB_arrays(threadid)%initialize(lmP_max_dtB)
...
!------------------------------------------------------------------------------
   subroutine do_iteration_ThetaBlocking_OpenMP(this,nR,nBc,time,timeStage,   &
      ...
      print *, " * Deprecated function do_iteration_ThetaBlocking_OpenMP, doing nothing!!"
