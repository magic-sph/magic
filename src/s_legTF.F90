...
!----- Defining theta block
    INTEGER :: nThetaStart,sizeThetaB

!------ Legendre Polynomials in c_horizontal.f
    INTEGER :: l_max,minc,lm_max
    REAL(kind=8) :: Plm(lm_max,*)
    REAL(kind=8) :: dPlm(lm_max,*)
...
!           and equatorially symmetric and antisymmetric contribution
    INTEGER :: ncp
    COMPLEX(kind=8) :: vrc(ncp,*),vrES,vrEA
    COMPLEX(kind=8) :: vtc(ncp,*)
    COMPLEX(kind=8) :: vpc(ncp,*)
    COMPLEX(kind=8) :: cvrc(ncp,*),cvrES,cvrEA
    COMPLEX(kind=8) :: cvtc(ncp,*)
    COMPLEX(kind=8) :: cvpc(ncp,*)
