...
!-- local variables:
    integer :: n_f,n_cheb
    real(kind=8) :: fac_cheb_even
    real(kind=8) :: fac_cheb_odd
...
!----- recursion
    do n_cheb=n_cheb_max-1,1,-1
        fac_cheb_odd=d_fac*dble(4*n_cheb)
        fac_cheb_even=d_fac*dble(4*n_cheb-2)
        do n_f=n_f_start,n_f_stop
            df(n_f,n_cheb)=df(n_f,n_cheb+1) + &
                           fac_cheb_odd*f(n_f,n_cheb+1)
            ddf(n_f,n_cheb)=ddf(n_f,n_cheb+1) + &
                            fac_cheb_even*df(n_f,n_cheb)
        end do
    end do
